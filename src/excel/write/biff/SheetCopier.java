...
class SheetCopier
{
  ...
  public void copySheet()
  {
    ...
    excel.read.biff.ColumnInfoRecord[] readCirs = fromSheet.getColumnInfos();

    for (int i = 0 ; i < readCirs.length; i++)
    {
      excel.read.biff.ColumnInfoRecord rcir = readCirs[i];
    ...
    try
    {
      excel.read.biff.RowRecord[] rowprops  = fromSheet.getRowProperties();
    ...
    if (fromSheet.getPLS() != null)
    {
      ...
      {
        System.out.println("Cannot copy Biff7 print settings record - ignoring");
      }
      else
      {
  ...
  public void importSheet()
  {
    ...
    try
    {
      ...
      for (int i = 0; i < rowprops.length; i++)
      {
        ...
        excel.read.biff.RowRecord rowrec = rowprops[i];
  ...
  private WritableCell deepCopyCell(Cell cell)
  {
    ...
    if (c instanceof ReadFormulaRecord)
    {
      ...
      if (crossSheetReference)
      {
        try
        {
        System.out.println("Formula " + rfr.getFormula() +
                    " in cell " + 
        ...
        {
          System.out.println("Formula  in cell " + 
                      CellReferenceHelper.getCellReference(cell.getColumn(),
                                                           cell.getRow()) +
    ...
    for (int i = 0 ; i < nameRecords.length ;i++)
    {
      ...
      for (int j = 0; j < nameRanges.length; j++)
      {
        ...
        if (fromSheetIndex == nameSheetIndex)
        {
          ...
          {
            System.out.println("Named range " + name + 
