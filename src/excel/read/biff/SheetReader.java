...
import excel.biff.AutoFilterInfoRecord;
import excel.biff.AutoFilterRecord;
import excel.biff.ConditionalFormat;
import excel.biff.ConditionalFormatRangeRecord;
import excel.biff.ConditionalFormatRecord;
import excel.biff.ContinueRecord;
import excel.biff.DataValidation;
import excel.biff.DataValidityListRecord;
import excel.biff.DataValiditySettingsRecord;
import excel.biff.FilterModeRecord;
...
final class SheetReader
{
  ...
  private void addCell(Cell cell)
  {
    ...
    if (cell.getRow() < numRows && cell.getColumn() < numCols)
    {
      if (cells[cell.getRow()][cell.getColumn()] != null)
      {
        ...
        System.out.println("Cell " + sb.toString() +
  ...
  final void read()
  {
    ...
    while (cont)
    {
      ...
      {
        System.out.println("Biff code zero found");

        // Try a dimension record
        if (r.getLength() == 0xa)
        {
          System.out.println("Biff code zero found - trying a dimension record.");
          r.setType(Type.DIMENSION);
        }
        else
        {
          System.out.println("Biff code zero found - Ignoring.");
        }
      }
      ...
      else if (type == Type.SHAREDFORMULA)
      {
        ...
        {
          System.out.println("Shared template formula is null - " +
      ...
      else if (type == Type.DVAL)
      {
        if (!workbookSettings.getCellValidationDisabled())
        {
          ...
          {
            System.out.println("object id " + dvlr.getObjectId() + " referenced " +
      ...
      else if (type == Type.DV)
      {
        if (!workbookSettings.getCellValidationDisabled())
        {
          ...
          {
            System.out.println("cannot add data validity settings");
          }
        }
      }
    ...
    if (!comments.isEmpty())
    {
      System.out.println("Not all comments have a corresponding Note record");
    }
  }
                              ...
                              double height)
  {
    ...
    if (c == null)
    {
      System.out.println("Cell at " + CellReferenceHelper.getCellReference(col, row) +
    ...
    {
      System.out.println("Not able to add comment to cell type " +
                  c.getClass().getName() +
