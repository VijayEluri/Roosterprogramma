...
import excel.biff.BaseCompoundFile;
...
public final class CompoundFile extends BaseCompoundFile
{
  ...
  private void readSmallBlockDepot() throws BiffException
  {
    ...
    if (sbdBlock == -1)
    {
        System.out.println("invalid small block depot number");
      return;
    }
      ...
      if (ps.name == null || ps.name.length() == 0)
      {
        if (ps.type == ROOT_ENTRY_PS_TYPE)
        {
          ps.name = ROOT_ENTRY_NAME;
            System.out.println("Property storage name for " + ps.type +
        ...
        else
        {
          if (ps.size != 0)
          {
              System.out.println("Property storage type " + ps.type +
    ...
    throws BiffException
  {
    ...
    if (multiple)
    {
        System.out.println("found multiple copies of property set " + name);
    }
  ...
  private byte[] getBigBlockStream(PropertyStorage ps)
  {
    ...
    {
        System.out.println("Property storage size inconsistent with block chain.");
    }
    ...
    throws BiffException
  {
    ...
    for (; blockCount <= smallBlockChain.length && block != -2; blockCount++)
    {
      ...
      if (block == -1)
      {
        System.out.println("Incorrect terminator for small block stream " + ps.name);
