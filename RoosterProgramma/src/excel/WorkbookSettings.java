...
import excel.biff.CountryCode;
import excel.biff.formula.FunctionNames;
...
public final class WorkbookSettings
{
   ...
   * Drawings deactivated using -Dexcel.nodrawings=true on the JVM command line
   * Activated by default or by using -Dexcel.nodrawings=false on the JVM command
   * line
   */
   ...
   * Names deactivated using -Dexcel.nonames=true on the JVM command line
   * Activated by default or by using -Dexcel.nonames=false on the JVM command
   * line
   */
   ...
   * GC deactivated using -Dexcel.nogc=true on the JVM command line
   * Activated by default or by using -Dexcel.nogc=false on the JVM command line
   */
   ...
   * performance reasons.  It can be deactivated using -Dexcel.norat=true on
   ...
   * country System properties eg. -Dexcel.lang=en -Dexcel.country=UK for UK
   * English
   */
   ...
   * from abroad.  This may also be set using the system property excel.encoding
   */
  ...
  public WorkbookSettings()
  {
    ...
    try
    {
      drawingsDisabled        = Boolean.getBoolean("excel.nodrawings");
      namesDisabled           = Boolean.getBoolean("excel.nonames");
      gcDisabled              = Boolean.getBoolean("excel.nogc");
      rationalizationDisabled = Boolean.getBoolean("excel.norat");
      mergedCellCheckingDisabled =
        Boolean.getBoolean("excel.nomergedcellchecks");
      formulaReferenceAdjustDisabled =
                                Boolean.getBoolean("excel.noformulaadjust");
      propertySetsDisabled = Boolean.getBoolean("excel.nopropertysets");
      ignoreBlankCells = Boolean.getBoolean("excel.ignoreblanks");
      cellValidationDisabled = Boolean.getBoolean("excel.nocellvalidation");
      autoFilterDisabled = !Boolean.getBoolean("excel.autofilter"); 
             // autofilter currently disabled by default
      useTemporaryFileDuringWrite = 
        Boolean.getBoolean("excel.usetemporaryfileduringwrite");
      String tempdir =
        System.getProperty("excel.temporaryfileduringwritedirectory");
    ...
    try
    {
      if (System.getProperty("excel.lang")    == null ||
          System.getProperty("excel.country") == null)
      {
        locale = Locale.getDefault();
      }
      else
      {
        locale = new Locale(System.getProperty("excel.lang"),
                            System.getProperty("excel.country"));
      }

      if (System.getProperty("excel.encoding") != null)
      {
        encoding = System.getProperty("excel.encoding");
      }
    } 
    catch (SecurityException e)
    {
   ...
   * Leaving this feature enabled will result in the excel process using
   * more memory
   *
