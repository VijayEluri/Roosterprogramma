...
class WritableSheetImpl implements WritableSheet
{
    ...
    throws WriteException, RowsExceededException
  {
    ...
    if (cv.isReferenced())
    {
      throw new excelWriteException(excelWriteException.cellReferenced);
    }
        ...
        curSharedValidation)
    {
      ...
      System.out.println("Cannot add cell at " + 
  ...
  public void setColumnView(int col, CellView view)
  {
    ...
    {
      System.out.println("Maximum number of format records exceeded.  Using " +
                  "default format.");
   ...
   * @exception excel.write.biff.RowsExceededException
   * @deprecated use the override which takes a CellView object
   */
   ...
   * @exception excel.write..WriteException
   * @exception excel.write.biff.RowsExceededException
   */
    ...
    throws WriteException, RowsExceededException
  {
    ...
    {
      System.out.println("Cannot merge cells - top left and bottom right "+
    ...
    throws WriteException, RowsExceededException 
  {
    if (row2 < row1)
    {
      System.out.println("Cannot merge cells - top and bottom rows incorrectly " + 
                  "specified");
    }
  ...
  private String validateName(String n)
  {
    ...
    {
      System.out.println("Sheet name " + n + " too long - truncating");
      n = n.substring(0, maxSheetNameLength);
    }

    if (n.charAt(0) == '\'')
    {
      System.out.println("Sheet naming cannot start with \' - removing");
      n = n.substring(1);
    }
  ...
  void removeDataValidation(CellValue cv)
  {
    ...
    if (validatedCells != null)
    {
      ...
      if (!result)
      {
        System.out.println("Could not remove validated cell " + 
                    CellReferenceHelper.getCellReference(cv));
      }
    }
  }
    ...
    throws WriteException
  {
    ...
    {
      System.out.println("Cannot extend data validation for " +
    ...
    for (int y = startRow; y <= endRow; y++)
    {
      if (rows[y] != null)
      {
        ...
        for (int x = startColumn; x <= endCol; x++)
        {
          ...
          {
            System.out.println("Cannot apply data validation from " +
 ...
 /**
    ...
    throws WriteException
  {
    ...
    if (dvp.extendedCellsValidation())
    {
      ...
      {
        System.out.println("Cannot remove data validation from " +
                    CellReferenceHelper.getCellReference(dvp.getFirstColumn(),
                                                         dvp.getFirstRow()) +
                    "-" +
                    CellReferenceHelper.getCellReference(dvp.getLastColumn(),
