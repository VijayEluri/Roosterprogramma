...
public class WritableWorkbookImpl extends WritableWorkbook
    ...
    /**
     * The excel.common.workbook styles
     */
    ...
    public WritableWorkbookImpl(OutputStream os,
        ...
        if (wp.getExternalSheetRecord() != null) {
            ...
            excel.read.biff.SupbookRecord[] readsr = wp.getSupbookRecords();
            supbooks = new ArrayList(readsr.length);

            for (int i = 0; i < readsr.length; i++) {
                excel.read.biff.SupbookRecord readSupbook = readsr[i];
                if (readSupbook.getType() == excel.read.biff.SupbookRecord.INTERNAL
                        || readSupbook.getType() == excel.read.biff.SupbookRecord.EXTERNAL) {
                    supbooks.add(new SupbookRecord(readSupbook, settings));
                } else {
                    if (readSupbook.getType() != excel.read.biff.SupbookRecord.ADDIN) {
                        System.out.println("unsupported supbook type - ignoring");
                    }
                }
            }
        }
        ...
        if (!settings.getNamesDisabled()) {
            excel.read.biff.NameRecord[] na = wp.getNameRecords();
            ...
            for (int i = 0; i < na.length; i++) {
                if (na[i].isBiff8()) {
                    ...
                    System.out.println("Cannot copy Biff7 name records - ignoring");
                }
            }
        }
     ...
     *
     * @exception IOException
     * @exception excelWriteException
     */
    public void close() throws IOException, excelWriteException {
    ...
    public void write() throws IOException {
        ...
        WindowProtectRecord wpr = new WindowProtectRecord(settings.getWindowProtected());
        ...
        for (int i = 0; i < getNumberOfSheets(); i++) {
            ...
            // boundsheet record
            outputFile.setData(IntegerHelper.getFourBytes(outputFile.getPos()),
                    boundsheetPos[i] + 4);
    ...
    private int getInternalSheetIndex(String name) {
        int index = -1;
        String[] namecollection = getSheetNames();
        for (int i = 0; i < namecollection.length; i++) {
            if (name.equals(namecollection[i])) {
                index = i;
                break;
            }
        }
    ...
    public String getExternalSheetName(int index) {
        ...
        System.out.println("Unknown Supbook 1");
        return "[UNKNOWN]";
    }
    ...
    public String getLastExternalSheetName(int index) {
        ...
        // An unknown supbook - return unkown
        System.out.println("Unknown Supbook 2");
        return "[UNKNOWN]";
    }
     ...
     */
    public excel.read.biff.BOFRecord getWorkbookBof() {
        return null;
    }
    ...
    public int getExternalSheetIndex(String sheetName) {
        ...
        if (found) {
            ...
            if (supbook.getType() != SupbookRecord.INTERNAL
                ...
                System.out.println("Cannot find sheet " + sheetName + " in supbook record");
            }
    ...
    public void removeRangeName(String name) {
        ...
        if (found) {
            ...
            if (nameRecords.remove(name) == null) {
                System.out.println("Could not remove " + name + " from index lookups");
            }
        }
    }

    /**
     * Accessor for the excel.common.styles
     *
