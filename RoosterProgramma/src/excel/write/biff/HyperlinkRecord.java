...
public class HyperlinkRecord extends WritableRecordData
{
  ...
  protected HyperlinkRecord(Hyperlink h, WritableSheet s)
  {
    ...
    if (h instanceof excel.read.biff.HyperlinkRecord)
    {
  ...
  {
    excel.read.biff.HyperlinkRecord hl = (excel.read.biff.HyperlinkRecord) h;
  ...
  public byte[] getData()
  {
    ...
    // Build up the excel.common.data
   ...
   *
   * @param cd the data excel.common.for all types of hyperlink
  ...
  private byte[] getFileData(byte[] cd)
  {
    ...
    // Copy across the excel.common.data into the new array
    byte[] d = new byte[dataLength];
